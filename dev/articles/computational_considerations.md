...
`amadeus` relies on spatial tools such as `terra` and `exactextractr`.
These packages use efficient C++ implementations for raster, vector, and
extraction operations, so each individual operation is already optimized
...
parallelize naturally because dates, variables, and location chunks can
...
objects should not be returned across worker boundaries. Instead, write
each worker result to a file path and load those files back in the
parent process.
...
A `_targets.R` file can make date grids explicit and skip work that is
already up to date.
...
- [`terra::SpatRaster`](https://rspatial.github.io/terra/reference/SpatRaster-class.html)
  objects cannot safely cross worker boundaries; pass file paths between
  workers and the parent process instead.
...
- Aggregate disk usage can grow quickly. Use a worker-specific
  [`tempdir()`](https://rdrr.io/r/base/tempfile.html) path and clean up
  intermediate files when they are no longer needed.
- For very large grids, Dask or spatial chunking with
  [`terra::makeTiles()`](https://rspatial.github.io/terra/reference/makeTiles.html)
  may outperform process-level parallelism.
